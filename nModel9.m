...
% sum=[40,80,90,100,60,75,120,150,60,100,120,200,60,75,120,150,40,80,90,100]'
%   mul(:,1)=[ones(4,1)*3;ones(4,1)*2;ones(12,1)*1];
%   mul(:,2)=[ones(12,1)*1;ones(4,1)*2;ones(4,1)*3];
% if con==3
% data_ot=data_C;
% data_pl=data_M;
% end
...
result = nmodel_9(bootData);
results_ot.params(n,1:6) =  [n, 0, result.params]; 
results_ot.confirm(n,1:5) =  [n, 1, result.pseudoR2,result.loglike,result.bic]; 
display(n)
end
save([path,'nmodel_thesis_9_ot_',condition{con}],'results_ot');
end
...
result = nmodel_9(bootData);
results_pl.params(n,1:6) =  [n, 0, result.params]; 
results_pl.confirm(n,1:5) =  [n, 1, result.pseudoR2,result.loglike,result.bic]; 
display(n)
end
save([path,'nmodel_thesis_9_pl_',condition{con}],'results_pl');
end