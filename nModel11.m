...
for sub=1:size(mo,1)
   clearvars pos i
   pos=find(isnan(mo(sub,:))==1);
   for i=1:length(pos)
   clearvars type 
    type=find((mul(:,1)==mul(pos(i),1) & mul(:,2)==mul(pos(i),2))==1);
    mo(sub,pos(i))=sum(pos(i))*nanmean(mo(sub,type)./sum(type)');
   end
end
mo=round(mo);
...
save([path,'nmodel_thesis_11_pl_',condition{con}],'results_pl');
end